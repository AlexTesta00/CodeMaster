...
const handleConfirm = async () => {
    ...
    if (codequestToDelete) {
        ...
        codequests.value = codequests.value.filter(
            (q) => q.id !== codequestToDelete.value,
        )
    }
}
...
onMounted(async () => {
    ...
    if (auth.nickname) {
        try {
            ...
            if (response.success) {
                users.value = response.user.filter(
                    (user) => user.userInfo.nickname.value !== auth.nickname,
                )
    ...
    >
        ...
        >
            ...
            <div class="overflow-x-auto w-full py-8">
                <div
                    class="flex flex-nowrap gap-4 px-2"
                    v-if="users.length > 0"
                >
                    <user-card
                        v-for="user in users"
                        ...
                        :image="
                            isSome(user.profilePicture)
                                ? user.profilePicture.value.url
                                : '/images/barney.png'
                        "
                ...
                <p
                    v-else
                    class="text-center to-error dark:text-gray-400 text-lg italic"
                >
                    No Users Found
                </p>
            </div>
