...
const toggleFilterActive = (filter: string) => {
    ...
    if (filterActive.value === 'none') {
        ...
        codequests.value = allCodequests.value.filter(
            (quest, _) =>
                quest.codequest.difficulty.name.toLowerCase() ===
                filterActive.value,
        )
    }
}
...
onMounted(async () => {
    ...
    if (codequestRes.success) {
        for (const codeQuest of codequestRes.codequests) {
            ...
            if (solRes.success) {
                const userSolution = solRes.solutions.find(
            ...
            easyCod.value = allCodequests.value.filter(
                (quest) =>
                    quest.codequest.difficulty.name.toLowerCase() == 'easy' &&
                    quest.isSolved,
            ).length
            mediumCod.value = allCodequests.value.filter(
                (quest) =>
                    quest.codequest.difficulty.name.toLowerCase() == 'medium' &&
                    quest.isSolved,
            ).length
            hardCod.value = allCodequests.value.filter(
                (quest) =>
                    quest.codequest.difficulty.name.toLowerCase() == 'hard' &&
                    quest.isSolved,
            ).length
            easyCodMax.value = allCodequests.value.filter(
                (quest) =>
                    quest.codequest.difficulty.name.toLowerCase() == 'easy',
            ).length
            mediumCodMax.value = allCodequests.value.filter(
                (quest) =>
                    quest.codequest.difficulty.name.toLowerCase() == 'medium',
            ).length
            hardCodMax.value = allCodequests.value.filter(
                (quest) =>
                    quest.codequest.difficulty.name.toLowerCase() == 'hard',
            ).length
        }
