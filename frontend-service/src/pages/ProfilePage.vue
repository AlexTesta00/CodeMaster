...
const removeLanguage = async (lang: string) => {
    ...
    if (auth.nickname) {
        try {
            ...
            await errorToast(
                'Languages not updated - service temporarily unavailable',
            )
        }
    }
}
...
const logout = async () => {
    ...
    try {
        ...
        await errorToast(
            'User not logged out - service temporarily unavailable',
        )
    } finally {
        loading.value = false
    }
...
onMounted(async () => {
    ...
    try {
        ...
        if (response.success) {
            codequests.value = response.codequests.filter(
                (quest) => quest.author == auth.nickname,
            )
        }
    } catch (error) {
