...
const debug = async () => {
    ...
    try {
        ...
        if (
            ...
            const testCode = availableLanguages.value.find(
                (l) => l.language === lang,
            )?.testCode
            const languageCode = solution.value.codes.find(
                (code) => code.language.name == lang,
            )
            if (testCode && languageCode) {
                const response = await debugCode(solution.value.id, testCode, {
        ...
        try {
            ...
            if (id) {
                ...
                if (questRes.success) {
                    ...
                    solution.value = solRes.solutions.find(
                        (sol) => sol.user === auth.nickname,
                    )
                    if (solRes.success && solution.value && codeRes.success) {
                        availableLanguages.value = solution.value.codes.map(
                            (code) => {
                                const matched =
                                    codeRes.generatedCodes.entries.find(
                                        (langCode) =>
                                            langCode.language ==
                                            code.language.name,
                                    )
                                if (!matched) {
                                    throw new Error(
                                        `No code found for language ${code.language.name}`,
                                    )
                                }
                                return {
                        ...
                        originalCodes.value = availableLanguages.value.map(
                            (code) => ({
                        ...
                        if (codeRes.success) {
                            availableLanguages.value =
                                codeRes.generatedCodes.entries
                            currentLanguage.value = availableLanguages.value[0]
                            originalCodes.value = availableLanguages.value.map(
                                (code) => ({
                                    language: code.language,
...
const resetCurrentCode = () => {
    if (currentLanguage.value) {
        const original = originalCodes.value.find(
            (l) => l.language === currentLanguage.value!.language,
        )
        if (original) {
            ...
            if (idx !== -1) {
                availableLanguages.value[idx].templateCode =
    ...
    >
        ...
        >
            <markdown-viewer
                ...
                class="w-full h-full"
            />
            <comments-panel v-if="codequest" :quest-id="codequest.id" />
        </div>
        ...
        >
            ...
            <div v-if="isTerminalOpen" class="h-60 bg-black overflow-y-auto">
                <div class="p-4">
                    <div v-if="isExecuting" class="flex items-center space-x-2">
                        ...
                        <span class="text-white" v-if="executionType === 'run'"
                            >Running code...</span
                        >
            ...
            <footer class="h-20 w-full z-20">
                <div
                    class="w-full h-full bg-primary rounded-lg flex flex-row justify-evenly items-center flex-wrap"
                >
