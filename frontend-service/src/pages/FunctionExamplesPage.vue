...
const canSubmit = computed(() => {
    return examples.value.every(
        (example) =>
            example.inputs.length === parameters.value.length &&
            example.inputs.every((input, i) =>
                isValidInput(input, parameters.value[i].typeName),
            ) &&
...
const proceed = async () => {
    if (!canSubmit.value) {
        await errorToast(
            'Please complete all fields and fix any example errors before continuing.',
        )
        return
    }
    ...
    () => {
        exampleErrors.value = examples.value.map((ex) => {
            const inputsErrors = ex.inputs.map((input, i) =>
                isValidInput(input, parameters.value[i].typeName)
                    ? null
                    : `Input ${parameters.value[i].name} not valid`,
            )
            const outputError = isValidOutput(ex.output, returnType.value)
                ? null
                : 'Output not valid'
            return {
                inputs: inputsErrors,
...
<template>
    ...
    >
        <label class="text-2xl font-bold mb-6">
            Define Examples for
            <span class="text-primary">{{ functionName }}</span>
        </label>
        ...
        >
            <label class="text-xl font-semibold mb-4 text-black dark:text-white"
            ...
            >
                <label
                    class="block text-md font-medium text-black dark:text-white mb-1"
                >
                ...
                <p
                    v-if="
                        exampleErrors[index] &&
                        exampleErrors[index].inputs.every((i) => i)
                    "
                    class="text-error text-base"
                >
                    {{ exampleErrors[index].inputs[0] }}
                </p>
            </div>
            ...
            <div class="mb-4">
                ...
                />
                <p v-if="exampleErrors[index]" class="text-error text-base">
                    {{ exampleErrors[index].output }}
                </p>
            </div>
