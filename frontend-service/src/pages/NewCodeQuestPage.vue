...
const validateDescription = () => {
    descriptionError.value =
        description.value.trim() === '' ? "Description can't be empty" : null
}

const validateConstraints = () => {
    constraintsError.value =
        constraints.value.trim() === '' ? "Constraints can't be empty" : null
}
...
onMounted(() => {
    ...
    if (codeQuestStore.description)
        description.value = codeQuestStore.description
    if (codeQuestStore.constraints)
        constraints.value = codeQuestStore.constraints
    if (codeQuestStore.languages.length > 0)
        languages.value = [...codeQuestStore.languages]
    if (codeQuestStore.functionName)
        functionName.value = codeQuestStore.functionName
    if (codeQuestStore.parameters.length > 0)
        parameters.value = [...codeQuestStore.parameters]
...
<template>
    ...
    >
        ...
        >
            ...
            <label for="constraints" class="text-black dark:text-white text-2xl"
                >Constraints*</label
            >
            <textarea
                id="constraints"
                ...
                required
            />
            <p v-if="constraintsError" class="text-red-500 text-sm mt-1">
                {{ constraintsError }}
            </p>
            ...
            >
            <div
                class="w-full flex flex-row items-center justify-start gap-4 text-black dark:text-white text-2xl"
            >
                <select
                    v-model="difficulty"
    ...
    >
        ...
        <button-with-image
            ...
            @click="
                () => {
                    if (validateForm()) isQuestDialogOpen = true
                }
            "
        />
    </footer>
