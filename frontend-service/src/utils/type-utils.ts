...
export const isValidPrimitive = (
    value: string,
    type: AllowedTypeName,
): boolean => {
    switch (type) {
        case 'int':
            return /^-?\d+$/.test(value)
...
export const isValidInput = (input: string, type: AllowedTypeName) =>
    isValidValue(input, type)
export const isValidOutput = (output: string, type: AllowedTypeName) =>
    isValidValue(output, type)
