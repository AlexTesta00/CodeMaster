...
const zippedTests = computed(() => {
    if (
        props.result.type === 'accepted' ||
        props.result.type === 'testsFailed'
    ) {
    ...
    if (
        props.result.type === 'compileFailed' ||
        props.result.type === 'runtimeError'
    ) {
        return [
            {
                input: '',
...
<template>
    <div
        v-if="result"
        class="p-4 text-white rounded-lg font-mono whitespace-pre-wrap"
    >
        <template v-if="showTestResults">
            ...
            >
                ...
                <template v-else>
                    ...
                    <div>
                        <strong v-if="executionType === 'run'"
                            >Output ottenuto:</strong
                        >
                        {{ test.actualOutput }}
                    </div>
                </template>
