...
const parameters = ref<FunctionParameter[]>([{ name: '', typeName: 'int' }])
const returnType = ref<AllowedTypeName>('int')
const examples = ref<FunctionExample[]>([{ inputs: [], output: '' }])

const allowedTypes = [...primitiveTypes] as const
...
const validate = () => {
    ...
    if (!functionName.value.trim()) {
        ...
        errors.value.functionName =
            'Invalid function name. CamelCase, no spaces, no digits first.'
        valid = false
    } else {
    ...
    if (parameters.value.length === 0) {
        ...
        errors.value.parameters = parameters.value.map((param) => {
            ...
            if (!isValidIdentifier(name))
                return 'Invalid parameter name. CamelCase, no spaces, no digits first.'
    ...
    () => {
        examples.value.forEach((e) => {
            while (e.inputs.length < parameters.value.length) {
                ...
                let defaultVal = ''
                if (t.startsWith('int') || t.startsWith('double'))
                    defaultVal = '0'
...
<template>
    <label class="text-black dark:text-white text-2xl"
    ...
    <input
        ...
        required
    />
    <p v-if="errors.functionName" class="text-error text-base">
        {{ errors.functionName }}
    </p>
    ...
    <label for="allowedTypes" class="text-black dark:text-white text-2xl"
        >Return Type*</label
    >
    <div
