import {
  CommentError,
  CommentFactory,
} from '../../main/nodejs/codemaster/servicies/community/domain/comment-factory'
...
describe('CommentFactoryTest', () => {
  ...
  it(
    'should throw error if questId is invalid',
    () => {
      const invalidQuestId = ''
      expect(() => CommentFactory.newComment(content, invalidQuestId, author)).toThrow(
        CommentError.InvalidQuestId
      )
    },
    timeout
  )
  ...
  it(
    'should throw error if author name is invalid',
    () => {
      const invalidAuthor = ''

      expect(() => CommentFactory.newComment(content, questId, invalidAuthor)).toThrow(
        CommentError.InvalidAuthor
      )
    },
    timeout
  )
  ...
  it(
    ...
    () => {
      ...
      expect(() => CommentFactory.newComment(invalidContent, questId, author)).toThrow(
