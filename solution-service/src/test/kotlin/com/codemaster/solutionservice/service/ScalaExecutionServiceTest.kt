...
class ScalaExecutionServiceTest : DescribeSpec() {
            ...
            context("Compile and execute solution code") {
                ...
                it("should fail if the compilation is not successful") {
                    ...
                    result.shouldBeTypeOf<ExecutionResult.CompileFailed>().also {
                        ...
                        it.exitCode shouldBe 1
                        it.stderr shouldContain "error: Missing closing brace `}' assumed here\n" +
                                "    s\n" +
                                "    ^\n" +
                                "one error found"
                        it.stderr shouldContain "Main.scala"
                    }
                }
                ...
                it("should fail execution if there is a runtime exception") {
                    ...
                    val newCode = """
                      def myPrint(input: String): String = {
                        input.toUpperCase()
                      }
                    """.trimIndent()
