...
export class AuthenticationServiceImpl implements AuthenticationService {
  ...
  async registerUser(nickname: UserId, email: string, password: string): Promise<User> {
    if (!Validator.isValidPassword(password)) {
      throw new AuthenticationServiceError.InvalidPasswordFormat(
    ...
    await this.userRepository.save(newUser).catch(() => {
      throw new AuthenticationServiceError.UserAlreadyExist('User already exist')
    })
    return newUser
  }
  ...
  async deleteUser(id: UserIdentifier): Promise<void> {
    const user: User = await this.#verifyUser(id)
    await this.userRepository.deleteUser(user.id).catch(() => {
  ...
  async updateUserEmail(id: UserIdentifier, newEmail: string): Promise<void> {
    ...
    const user: User = await this.#verifyUser(id)
    await this.userRepository.updateUserEmail(user.id, newEmail).catch(() => {
      throw new AuthenticationServiceError.InvalidCredential('User not found')
    })
  }
  ...
  async updateUserPassword(
    ...
    const hashedPassword: string =
      await BcryptService.generateHashForPassword(newPassword)
