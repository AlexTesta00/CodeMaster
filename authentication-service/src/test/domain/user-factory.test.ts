...
describe('TestUserFactory', () => {
    ...
    it('should throw error when create user with invalid email', async () => {
        expect(() => {UserFactory.createUser(userId, wrongEmail, password)}).toThrow(UserFactoryError.InvalidEmail);
    }, 10000);

    it('should throw error when create user with invalid nickname', async () => {
        expect(() => {UserFactory.createUser(wrongUserId, email, password)}).toThrow(UserFactoryError.InvalidNickname);
    }, 10000);
});