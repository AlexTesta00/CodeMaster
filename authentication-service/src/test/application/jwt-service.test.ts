...
import { AuthenticationServiceError } from '../../main/nodejs/codemaster/servicies/authentication/application/authentication-service'
...
describe('Test JWT Service', () => {
  ...
  describe('Test refresh token', () => {
    ...
    it(
      'should reject if refresh token is invalid',
      () => {
        const invalidToken = 'invalid.invalid'
        expect(() => jwtService.verifyRefreshToken(invalidToken)).toThrow(
          AuthenticationServiceError.InvalidRefreshToken
        )
      },
      timeout
    )
