...
describe('Test bcrypt service', () => {
  ...
  it(
    'should return different hash for same password',
    async () => {
      const differentHashForSamePassword: string =
        await BcryptService.generateHashForPassword(password)
  ...
  it(
    'should return true, because hashed password corresponds to clean password',
    async () => {
  ...
  it(
    'should return true, because hashed password not corresponds to clean password',
    async () => {
      const notSamePassword: string = 'abcd'
      expect(
        await BcryptService.isSamePassword(hashedPassword, notSamePassword)
      ).not.toBeTruthy()
    },
    timeout
  )
